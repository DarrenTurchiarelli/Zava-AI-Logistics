...
async def register_parcel_interactive():
    ...
    while True:
        ...
        destination_state = input("Destination state (NSW/VIC/QLD/SA/WA/TAS/ACT/NT): ").strip().upper()
        ...
        if destination_state not in valid_states:
            print(f"Invalid state. Please use one of: {', '.join(valid_states)}")
            continue
        ...
        weight_str = input("Weight (e.g., 2.5kg, 500g, 1.2) [optional]: ").strip()
        weight = None
        if weight_str:
            # Parse weight - accept kg, g, or just numbers (assume kg)
            weight_str_lower = weight_str.lower()
            if 'g' in weight_str_lower and 'kg' not in weight_str_lower:
                # Input is in grams
                try:
                    weight_grams = float(weight_str_lower.replace('g', '').strip())
                    weight = weight_grams / 1000  # Convert to kg
                    print(f"  Converted {weight_grams}g to {weight}kg")
                except ValueError:
                    print("  Invalid weight format. Skipping weight.")
                    weight = None
            elif 'kg' in weight_str_lower:
                # Input is in kg
                try:
                    weight = float(weight_str_lower.replace('kg', '').strip())
                except ValueError:
                    print("  Invalid weight format. Skipping weight.")
                    weight = None
            else:
        ...
        declared_value_str = input("Declared value (AUD $) [optional]: ").strip()
        ...
        try:
            ...
            # Ask if they want to add another parcel
            add_another = input("\nWould you like to add another parcel? (y/n): ").strip().lower()
...
async def view_recent_parcels():
    ...
    try:
        ...
        for i, parcel in enumerate(parcels[:10], 1):  # Show last 10 parcels
            ...
            print(f"    To: {parcel['recipient_name']} ({parcel['destination_postcode']}, {parcel.get('destination_state', 'N/A')})")
...
async def track_parcel():
    ...
    try:
        ...
        if parcel:
            ...
            print(f"State: {parcel.get('destination_state', 'N/A')}")
            print(f"Postcode: {parcel['destination_postcode']}")
            ...
            if parcel.get('declared_value'):
                print(f"Declared Value: AUD ${parcel['declared_value']}")
            if parcel.get('special_instructions'):
