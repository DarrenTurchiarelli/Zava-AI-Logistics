...
Context:
- Suspicious message received claiming to be from DT Logistics
...
"""
        ...
        cs_response = await customer_service_agent({
            "customer_name": context.customer_name,
            "inquiry": prompt,
            "context": customer_service_context
        })
    ...
    async def _step_3_send_notification(self, context: FraudWorkflowContext):
        ...
        notification_result = await self._send_customer_notification(
            ...
            context.customer_warning_message,
        ...
        if context.notification_sent:
            self._log_step(
                context,
                f"Notification sent via {context.notification_method}",
                "success"
            )
        else:
    ...
    async def _step_5_hold_parcels(self, context: FraudWorkflowContext):
        ...
        if context.suspicious_parcel_ids:
            ...
            context.escalation_reason = f"Fraud risk score: {context.fraud_risk_score:.0%}, Identity verification required"
            
            self._log_step(
                context,
                f"Held {len(context.parcels_held)} parcels for security review",
                "warning"
            )
        else:
...
We've detected a suspicious message claiming to be from DT Logistics that may be attempting to defraud you.
...
async def example_usage():
    ...
    result = await fraud_detection_to_customer_service_workflow(
        message_content="""
        URGENT: Your DT Logistics parcel delivery failed!
