...
@dataclass
class Store:
    """Represents a Zava distribution center or store"""
...
def create_store(
    ...
    """
    Create a new Zava store or distribution center
    ...
    return Store(
        ...
        parcels_in_store=[],
        drivers_assigned=[],
        statistics={"total_processed": 0, "total_delivered": 0, "total_failed": 0, "total_returned": 0},
    )
...
def get_store_capacity_status(store: Store) -> str:
    ...
    return (
        f"{store.store_id}: {store.current_inventory}/{store.capacity} parcels ({utilization_pct:.1f}%) - {status_msg}"
    )
...
if __name__ == "__main__":
    print("Zava - Store Management Module")
    print("=" * 50)
    ...
    store = create_store(
        ...
        store_name="Zava Melbourne Distribution Center",
