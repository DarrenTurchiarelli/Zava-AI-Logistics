"""
DT Logistics - Sequential Workflow with Human Approval
...
def run_workflow_with_approval():
    ...
    print("DT Logistics - Sequential Workflow with Human Approval")
    print("=" * 70)
    print()
    ...
    parcel_data = {
        ...
        "sender_name": "DT Logistics Melbourne DC",
...
"""
            ...
            project_client.agents.create_message(
                thread_id=thread.id,
                role="user",
                content=sorting_prompt
            )
            
            run = project_client.agents.create_and_process_run(
                thread_id=thread.id,
                assistant_id=sorting_agent_id
            )
...
"""
                ...
                run = project_client.agents.create_and_process_run(
                    thread_id=thread.id,
                    assistant_id=coordination_agent_id
                )
