"""
Zava - Manifest Generation Agent
...
from src.infrastructure.agents.core.base import dispatcher_agent
...
class ManifestGenerationAgent:
    ...
    async def generate_optimized_manifests(
        self, parcels: List[ManifestParcel], available_drivers: List[Driver], date: str, dc_location: str = None
        ...
        assignments = await self._assign_parcels_to_drivers(clusters, available_drivers, dc_location)
        ...
        unassigned = [
            p for p in parcels if not any(p.parcel_id in [mp.parcel_id for mp in m.parcels] for m in manifests)
        ]
    ...
    async def _cluster_parcels_geographically(self, parcels: List[ManifestParcel]) -> List[List[ManifestParcel]]:
    ...
    async def _assign_parcels_to_drivers(
        self, clusters: List[List[ManifestParcel]], drivers: List[Driver], dc_location: str
    ...
    async def _create_optimized_manifest(
        self, driver_id: str, driver: Driver, parcels: List[ManifestParcel], date: str, dc_location: str
    ) -> OptimizedManifest:
    ...
    def _calculate_overall_confidence(
        self, manifests: List[OptimizedManifest], all_parcels: List[ManifestParcel], all_drivers: List[Driver]
    ) -> float:
        ...
        # Combined confidence
        overall = avg_manifest_confidence * 0.5 + assignment_rate * 0.3 + balance_score * 0.2
...
async def demo_manifest_generation():
    ...
    sample_parcels = [
        ...
        ManifestParcel(
            "P007", "DTVIC007", "222 Smith St, Fitzroy VIC", "3065", 1, 18, requires_signature=True, value_dollars=1200
        ),
    ...
    result = await agent.generate_optimized_manifests(
        parcels=sample_parcels, available_drivers=sample_drivers, date="2025-12-08", dc_location="Melbourne CBD"
    )
