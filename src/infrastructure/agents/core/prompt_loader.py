...
Loads system prompts and skills documentation from skills folder structure.
...
Usage:
    from src.infrastructure.agents import get_agent_prompt, get_agent_skills
...
def get_agent_skills_folder() -> Path:
    ...
    Returns:
        Path object pointing to skills folder
    """
    # Get the current file's directory (core/)
    current_file = Path(__file__)
    # Go up to agents/, then into skills/
    agent_skills_folder = current_file.parent.parent / "skills"
    
    if not agent_skills_folder.exists():
        raise FileNotFoundError(
            f"Agent skills folder not found at: {agent_skills_folder}\n"
            "Expected structure: src/infrastructure/agents/skills/<agent-name>/system-prompt.md"
        )
