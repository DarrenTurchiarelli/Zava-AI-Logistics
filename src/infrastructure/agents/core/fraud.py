...
class FraudRiskAgent:
    ...
    def _load_educational_templates(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            "delivery_fee_scam": {
                ...
                "actions": [
                    "Check your official Zava account for real delivery status",
            ...
            "phishing": {
                ...
                "actions": [
                    ...
                    "Verify the request through official Zava website",
            ...
            "general": {
                ...
                "actions": [
                    ...
                    "Block the sender",
                    "Contact Zava directly if you have concerns",
                    "Report suspicious messages to help protect others",
                ],
            },
        }
    ...
    def _analyze_with_ai_sync(self, message_content: str, sender_info: str) -> ThreatAnalysis:
        ...
        try:
            # Connect to Azure OpenAI using token credential (no API key needed)
            ...
            # Create thread with message, run agent, wait for completion
            ...
            ai_response_obj = next(
                (m for m in messages.data if m.role == "assistant"), None
            )

            # Extract text from message content
            if ai_response_obj and ai_response_obj.content:
                content = ai_response_obj.content[0]
    ...
    def _parse_ai_response(self, ai_response: str, message_content: str, sender_info: str) -> ThreatAnalysis:
        ...
        if any(
            ...
            for phrase in ["none identified", "no risk indicators", "no threats", "appears to be legitimate"]
        ):
        ...
        alert_security = threat_level in [ThreatLevel.HIGH, ThreatLevel.CRITICAL] and confidence_score >= 0.95
    ...
    def _fallback_analysis(self, message_content: str, sender_info: str) -> ThreatAnalysis:
        ...
        # Zava impersonation
        if "Zava" in content_lower or "dt log" in content_lower:
            risk_indicators.append("Possible Zava impersonation attempt")
        ...
        # Suspicious sender patterns - flag if sender claims to be Zava but isn't
        if sender_info != "unknown" and "@" in sender_info:
            ...
            # Check if sender claims to be Zava (in message or sender name) but uses wrong domain
            claims_dt_logistics = "dt" in sender_lower or "logistics" in sender_lower or "Zava" in content_lower
            ...
            if claims_dt_logistics and not is_official_domain:
                risk_indicators.append(f"Sender impersonating Zava from unofficial domain: {sender_info}")
        ...
        print(
            f"📊 Fallback analysis: {threat_level.value} threat, {len(risk_indicators)} indicators, {confidence_score:.0%} confidence"
        )
        ...
        return ThreatAnalysis(
            ...
            related_patterns=[
                f"Analyzed {len(message_content)} characters",
                "Fallback analysis - Azure AI unavailable",
            ],
    ...
    def _generate_recommendations(self, fraud_category: FraudCategory, threat_level: ThreatLevel) -> List[str]:
        ...
        if threat_level == ThreatLevel.LOW:
            ...
            recommendations.append("Verify with Zava through official contact methods")
        ...
        if fraud_category in [FraudCategory.PAYMENT_SCAM, FraudCategory.DELIVERY_FEE_SCAM]:
            if threat_level in [ThreatLevel.HIGH, ThreatLevel.CRITICAL]:
                recommendations.extend(
                    [
                        ...
                        "Contact Zava customer service directly to verify",
                        "Check your official Zava account for real status",
                    ]
                )
            elif threat_level == ThreatLevel.MEDIUM:
                recommendations.append("Verify any payment requests through official Zava channels")
            ...
            if threat_level in [ThreatLevel.HIGH, ThreatLevel.CRITICAL]:
                recommendations.extend(
                    ["Do not provide passwords or login credentials", "Do not click verification links in this message"]
                )
        ...
        if threat_level == ThreatLevel.LOW and fraud_category == FraudCategory.UNKNOWN:
            recommendations = [
                ...
                "Contact Zava directly if you have concerns about your delivery",
            ]
    ...
    def _extract_indicators_from_content(self, message_content: str) -> List[str]:
        ...
        if "Zava" in content_lower or "dt log" in content_lower:
            indicators.append("Claims to be from Zava")
    ...
    def _get_educational_content(self, fraud_category: FraudCategory, threat_level: ThreatLevel) -> Dict[str, Any]:
        ...
        if threat_level == ThreatLevel.LOW:
            return {
                ...
                "actions": ["Proceed with normal caution", "Verify through official channels if uncertain"],
            }
    ...
    def format_analysis_report(self, analysis: ThreatAnalysis) -> str:
        ...
        threat_icons = {ThreatLevel.LOW: "🟢", ThreatLevel.MEDIUM: "🟡", ThreatLevel.HIGH: "🟠", ThreatLevel.CRITICAL: "🔴"}
