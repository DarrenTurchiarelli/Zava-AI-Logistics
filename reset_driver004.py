"""Reset driver-004 manifest optimization flag"""
import asyncio
from parcel_tracking_db import ParcelTrackingDB

async def reset_driver004():
    """Reset optimization for driver-004 manifest"""
    
    async with ParcelTrackingDB() as db:
        manifest_container = db.database.get_container_client('driver_manifests')
        
        # Find driver-004 manifest
        ...
        async for manifest in manifest_container.query_items(query=query):
            ...
            await manifest_container.replace_item(item=manifest['id'], partition_key=manifest['driver_id'])
            print(f"✅ Reset optimization for {manifest['driver_name']} ({manifest['id']})")
            print(f"   Total items: {manifest['total_items']}")

if __name__ == '__main__':
    asyncio.run(reset_driver004())
    print("\n✅ Done! Log in as driver004 to trigger route optimization!")
