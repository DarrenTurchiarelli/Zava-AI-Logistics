...
from agents.base import call_agent_sync, customer_service_agent, optimization_agent, sorting_facility_agent
...
class RouteOptimizationAgent:
    ...
    async def optimize_driver_route(self, driver_id: str, addresses: List[str]) -> RouteOptimization:
        ...
        route_conditions = {
            ...
            "stops": [{"address": addr, "sequence": i + 1} for i, addr in enumerate(addresses)],
            "traffic": "normal",
            "weather": "clear",
...
async def recalculate_route_eta():
    ...
    driver_routes = [
        {"driver": "DRV001", "addresses": ["Melbourne CBD, VIC", "Carlton, VIC", "Richmond, VIC", "St Kilda, VIC"]},
        {"driver": "DRV002", "addresses": ["Brunswick, VIC", "Fitzroy, VIC", "Collingwood, VIC"]},
    ]

    optimizations = []
    for route_info in driver_routes:
        optimization = await agent.optimize_driver_route(route_info["driver"], route_info["addresses"])
        if optimization:
...
class ExceptionResolutionAgent:
    ...
    def _format_customer_history(self, customer_history: Optional[Dict]) -> str:
        ...
        if customer_history.get("safe_place_enabled"):
            history_parts.append(
                f"Safe place delivery enabled: {customer_history.get('safe_place_location', 'not specified')}"
            )
    ...
    async def analyze_and_resolve(
        ...
        if not customer_msg or len(customer_msg) < 10:
            rule = self.resolution_rules.get(exception_type, {})
            customer_msg = rule.get(
                "default_message", "We're processing your delivery exception. You'll receive updates shortly."
            )
        ...
        debug_print(
            f"[Exception Resolution] Action: {recommended_action}, Confidence: {confidence_score:.0%}, Auto-exec: {auto_executable}"
        )
    ...
    def _apply_local_rule(
        self, exception_type: ExceptionType, rule: Dict, customer_history: Optional[Dict], weather_data: Optional[Dict]
    ) -> ExceptionResolution:
...
class SmartNotificationAgent:
    ...
    def __init__(self):
        self.channel_priorities = {"critical": "sms", "high": "sms", "medium": "email", "low": "email"}

    async def generate_proactive_notification(
        self, parcel_id: str, customer_name: str, customer_phone: str, context: Dict[str, Any]
    ) -> NotificationContext:
        ...
        channel = self._select_notification_channel(urgency, context.get("customer_preferences", {}))
    ...
    async def _generate_proactive_alert(self, context: Dict) -> str:
        ...
        if alert_type == "on_track":
            ...
            return (
                f"Your high-value item will arrive today. " f"Signature required. Please ensure someone is available."
            )
        else:
    ...
    def _select_notification_channel(self, urgency: str, customer_preferences: Dict) -> str:
