...
class StateManager:
    ...
    def record_agent_decision(self, decision: AgentDecision, success: bool = True) -> None:
        """
        ...
        if agent_key not in self.agent_metrics:
            self.agent_metrics[agent_key] = AgentPerformanceMetrics(
                agent_name=decision.agent_name, agent_type=decision.agent_type
            )
    ...
    def get_agent_dashboard_data(self) -> Dict[str, Any]:
        ...
        successful = sum(
            ...
            if any(m.successful_decisions > 0 and m.agent_name == d.agent_name for m in self.agent_metrics.values())
        )
    ...
    def transition_state(
        self, tracking_number: str, new_state: WorkflowState, agent_name: str = "System", notes: Optional[str] = None
    ) -> bool:
        """
    ...
    def _is_valid_transition(self, current: WorkflowState, target: WorkflowState) -> bool:
        ...
        valid_transitions = {
            ...
            WorkflowState.AWAITING_APPROVAL: [WorkflowState.APPROVED, WorkflowState.REJECTED, WorkflowState.RETURNED],
            WorkflowState.APPROVED: [WorkflowState.OUT_FOR_DELIVERY, WorkflowState.RETURNED],
            WorkflowState.REJECTED: [WorkflowState.AT_SORTING_FACILITY, WorkflowState.RETURNED],
            WorkflowState.OUT_FOR_DELIVERY: [
                WorkflowState.DELIVERED,
                WorkflowState.FAILED_DELIVERY,
                WorkflowState.RETURNED,
            ],
            WorkflowState.FAILED_DELIVERY: [WorkflowState.OUT_FOR_DELIVERY, WorkflowState.RETURNED],
    ...
    def process_approval(
        self, request_id: str, approved: bool, reviewer: str, review_notes: Optional[str] = None
    ) -> bool:
        """
    ...
    def get_pending_approvals(self) -> List[ApprovalRequest]:
        ...
        return [approval for approval in self.pending_approvals.values() if approval.status == "Pending"]
    ...
    def add_agent_message(self, tracking_number: str, agent_name: str, message: str) -> None:
        ...
        context = AgentContext(
            agent_name=agent_name, tracking_number=tracking_number, current_state=current_state, messages=[message]
        )
...
if __name__ == "__main__":
    print("Zava - State Manager Module")
    print("=" * 50)
